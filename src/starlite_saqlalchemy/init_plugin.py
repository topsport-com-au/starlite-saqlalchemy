...
class PluginConfig(BaseModel):
    ...
    """Add the [`Provide`][starlite.datastructures.Provide]'s for collection
    route dependencies to.
    ...
    """
    do_sentry: bool = True
    """Configure the application to initialize Sentry on startup. Adds a
    handler to.

    [`AppConfig.on_startup`][starlite.config.app.AppConfig.on_startup].
    """
    do_set_debug: bool = True
    """Allow the plugin to set the starlite `debug` parameter. Parameter set to
    value of.
    ...
    """
    do_worker: bool = True
    """Configure the async worker on the application. This action instantiates
    a worker instance and sets handlers for
