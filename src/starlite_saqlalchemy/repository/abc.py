...
class AbstractRepository(Generic[T], metaclass=ABCMeta):
    ...
    async def get_or_create(self, **kwargs: Any) -> tuple[T, bool]:
        """Get an instance specified by the ``kwargs`` filters if it exists or
        create it.
    ...
    async def update_many(self, data: list[T]) -> list[T]:
        """Update multiple instances with the attribute values present on
        instances in ``data``.
    ...
    def check_not_found(item_or_none: T | None) -> T:
        """Raise :class:`RepositoryNotFoundException` if ``item_or_none`` is
        ``None``.
    ...
    def get_id_attribute_value(cls, item: T) -> Any:
        """Get value of attribute named as :attr:`id_attribute.

        <AbstractRepository.id_attribute>` on ``item``.
    ...
    def set_id_attribute_value(cls, item_id: Any, item: T) -> T:
        """Return the ``item`` after the ID is set to the appropriate
        attribute.
