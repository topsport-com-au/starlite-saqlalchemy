...
def test_generic_mock_repository_raises_repository_exception_if_named_attribute_doesnt_exist(
    ...
    with pytest.raises(StarliteSaqlalchemyError):
        _ = author_repository.filter_collection_by_kwargs(
            author_repository.collection,
            cricket="ball",
        )
...
async def test_list_and_count() -> None:
    """Test that the repository list_and_count returns records and the total
    record count."""
...
async def test_get_one_or_none() -> None:
    """Test that the repository get_one_or_none returns a model record
    correctly."""
...
async def test_get_or_create() -> None:
    """Test that the repository get_or_create returns a model record
    correctly."""
